/*
 PlayCanvas Engine v0.147.7 revision 7e463c1afce1
 http://playcanvas.com
...
1),a._scene=null,this.updateShaders=!0)};d.prototype.update=function(){for(var a=0,b=this._models.length;a<b;a++)this._models[a].getGraph().syncHierarchy()};return{Scene:d}}());pc.extend(pc.scene,function(){function d(a,b){return a.distSqr&&b.distSqr?b.distSqr-a.distSqr:b.key-a.key}function a(a,b){var c=a.getNearClip(),e=a.getFarClip(),d=a.getFov()*Math.PI/180,f=a.getAspectRatio(),g=a.getProjection(),h,k;k=g===pc.scene.Projection.PERSPECTIVE?Math.tan(d/2)*c:this._orthoHeight;h=k*f;b[0].x=h;b[0].y=-k;b[0].z=-c;b[1].x=h;b[1].y=k;b[1].z=-c;b[2].x=-h;b[2].y=k;b[2].z=-c;b[3].x=-h;b[3].y=-k;b[3].z=-c;g===pc.scene.Projection.PERSPECTIVE&&(k=Math.tan(d/2)*e,h=k*f);b[4].x=h;b[4].y=
...
0;C<s.length;C++)x=s[C],x.getEnabled()&&(x.getType()===pc.scene.LIGHTTYPE_DIRECTIONAL?x.getCastShadows()?c._globalLights.push(x):c._globalLights.unshift(x):c._localLights[x.getType()===pc.scene.LIGHTTYPE_POINT?0:1].push(x));D=e.getPosition();C=0;for(numDrawCalls=A.length;C<numDrawCalls;C++)x=A[C],x.command||(x.material.blendType===pc.scene.BLEND_NORMAL?(x.syncAabb(),G=x.aabb.center,B=G.x-D.x,E=G.y-D.y,G=G.z-D.z,x.distSqr=B*B+E*E+G*G):"undefined"!==typeof x.distSqr&&delete x.distSqr);A.sort(d);if(e._depthTarget){D=
...
E._shadowResolution||H.height!==E._shadowResolution)H=b(B,E._shadowResolution,E._shadowResolution),G.setRenderTarget(H);B=G;if(D===pc.scene.LIGHTTYPE_DIRECTIONAL){D=e.getFrustumCentroid();B.setPosition(D);E=new pc.Vec3;x.worldTransform.getY(E);B.translate(E);B.lookAt(D);l.copy(B.getWorldTransform());a(e,h);D=l.invert();G=e.worldTransform;E=new pc.Mat4;E.mul2(D,G);for(D=0;8>D;D++)E.transformPoint(h[D],h[D]);E=1E6;G=-1E6;var H=1E6,I=-1E6,J=1E6,L=-1E6;for(D=0;8>D;D++){var N=h[D];N.x<E&&(E=N.x);N.x>G&&
(G=N.x);N.y<H&&(H=N.y);N.y>I&&(I=N.y);N.z<J&&(J=N.z);N.z>L&&(L=N.z)}B.translateLocal(0.5*-(G+E),0.5*(I+H),L+0.25*(L-J));l.copy(B.getWorldTransform());B.setProjection(pc.scene.Projection.ORTHOGRAPHIC);B.setNearClip(0);B.setFarClip(1.5*(L-J));B.setAspectRatio((G-E)/(I-H));B.setOrthoHeight(0.5*(I-H))}else D===pc.scene.LIGHTTYPE_SPOT&&(B.setProjection(pc.scene.Projection.PERSPECTIVE),B.setNearClip(x.getAttenuationEnd()/1E3),B.setFarClip(x.getAttenuationEnd()),B.setAspectRatio(1),B.setFov(2*x.getOuterConeAngle()),
l.mul2(x.worldTransform,k));n.copy(l).invert();r.mul2(B.getProjectionMatrix(),n);x._shadowMatrix.mul2(g,r);B.worldTransform.copy(l);f.setBlending(!1);f.setColorWrite(!0,!0,!0,!0);f.setCullMode(pc.gfx.CULLFACE_BACK);f.setDepthWrite(!0);f.setDepthTest(!0);f.extDepthTexture&&f.setColorWrite(!1,!1,!1,!1);this.setCamera(B);D=0;for(G=z.length;D<G;D++)x=z[D],B=x.mesh,E=x.material,this.modelMatrixId.setValue(x.node.worldTransform.data),E.opacityMap&&m.resolve("texture_opacityMap").setValue(E.opacityMap),
x.skinInstance?(this.poseMatrixId.setValue(x.skinInstance.matrixPalette),f.setShader(E.opacityMap?this._depthProgSkinOp:this._depthProgSkin)):f.setShader(E.opacityMap?this._depthProgStaticOp:this._depthProgStatic),x=x.renderStyle,f.setVertexBuffer(B.vertexBuffer,0),f.setIndexBuffer(B.indexBuffer[x]),f.draw(B.primitive[x])}this.setCamera(e);this.dispatchGlobalLights(c);this.dispatchLocalLights(c);c.fog!==pc.scene.FOG_NONE&&(this.fogColor[0]=c.fogColor.r,this.fogColor[1]=c.fogColor.g,this.fogColor[2]=
c.fogColor.b,this.fogColorId.setValue(this.fogColor),c.fog===pc.scene.FOG_LINEAR?(this.fogStartId.setValue(c.fogStart),this.fogEndId.setValue(c.fogEnd)):this.fogDensityId.setValue(c.fogDensity));C=0;for(numDrawCalls=A.length;C<numDrawCalls;C++)if(x=A[C],x.command)x.command();else{B=x.mesh;E=x.material;m=x.node.worldTransform;s=x.normalMatrix;m.invertTo3x3(s);s.transpose();this.modelMatrixId.setValue(m.data);this.normalMatrixId.setValue(s.data);x.skinInstance&&this.poseMatrixId.setValue(x.skinInstance.matrixPalette);
if(E!==K){E.shader||E.updateShader(f,c);f.setShader(E.shader);var K=E.parameters,P;for(P in K)m=K[P],m.scopeId||(m.scopeId=f.scope.resolve(P)),m.scopeId.setValue(m.data);this.alphaTestId.setValue(E.alphaTest);f.setBlending(E.blend);f.setBlendFunction(E.blendSrc,E.blendDst);f.setBlendEquation(E.blendEquation);f.setColorWrite(E.redWrite,E.greenWrite,E.blueWrite,E.alphaWrite);f.setCullMode(E.cull);f.setDepthWrite(E.depthWrite);f.setDepthTest(E.depthTest)}f.setVertexBuffer(B.vertexBuffer,0);x=x.renderStyle;
f.setIndexBuffer(B.indexBuffer[x]);f.draw(B.primitive[x]);K=E}}});return{ForwardRenderer:c}}());pc.extend(pc.scene,function(){var d=function(a){this.name=a||"Untitled";this._labels={};this.localPosition=new pc.Vec3(0,0,0);this.localRotation=new pc.Quat(0,0,0,1);this.localScale=new pc.Vec3(1,1,1);this.localEulerAngles=new pc.Vec3(0,0,0);this.position=new pc.Vec3(0,0,0);this.rotation=new pc.Quat(0,0,0,1);this.eulerAngles=new pc.Vec3(0,0,0);this.localTransform=new pc.Mat4;this.dirtyLocal=!1;this.worldTransform=new pc.Mat4;this.dirtyWorld=!1;this._right=new pc.Vec3;this._up=new pc.Vec3;this._forward=
...
point2d.z=point2d.z;return point2d},screenToWorld:function(a,b,c,e,d,g){"undefined"===typeof g&&(g=new pc.Vec3);var k=this.getProjectionMatrix(),l=this.getWorldTransform();this._viewMat.copy(l);this._viewMat.invert();this._viewProjMat.mul2(k,this._viewMat);k=this._viewProjMat.clone().invert();b=new pc.Vec3(2*(a/e)-1,2*((d-b)/d)-1,1);a=k.transformPoint(b);a.scale(1/(b.x*k.data[3]+b.y*k.data[7]+b.z*k.data[11]+k.data[15]));c/=this._farClip;g.lerp(this.getPosition(),a,c);return g},getAspectRatio:function(){return this._aspect},
