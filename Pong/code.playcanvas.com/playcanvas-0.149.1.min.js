/*
 PlayCanvas Engine v0.149.1 revision 94b3f6c58af6
 http://playcanvas.com
...
a.get_m_localPointA().y(),a.get_m_localPointA().z()),c=new pc.Vec3(a.get_m_localPointB().x(),a.get_m_localPointB().y(),a.get_m_localPointB().z()),d=new pc.Vec3(a.getPositionWorldOnA().x(),a.getPositionWorldOnA().y(),a.getPositionWorldOnA().z()),e=new pc.Vec3(a.getPositionWorldOnB().x(),a.getPositionWorldOnB().y(),a.getPositionWorldOnB().z()),a=new pc.Vec3(-a.get_m_normalWorldOnB().x(),-a.get_m_normalWorldOnB().y(),-a.get_m_normalWorldOnB().z());return new n(c,b,e,d,a)},_cleanOldCollisions:function(){for(var a in b)if(b.hasOwnProperty(a)){for(var d=
...
e&&(k=2);d?l=1:f&&(l=2);if(e=g[k][l])f=a.collision,this.hasEvent("contact")||(e&=-33),f.hasEvent("contact")||(e&=-2),f.hasEvent("collisionstart")||(e&=-3),f.hasEvent("collisionend")||(e&=-5),f.hasEvent("triggerenter")||(e&=-9),f.hasEvent("triggerleave")||(e&=-17);return e},onUpdate:function(a){this.dynamicsWorld.stepSimulation(a,this.maxSubSteps,this.fixedTimeStep);var b=this.store,d;for(d in b)if(b.hasOwnProperty(d)){var g=b[d].entity,k=b[d].data;k.body&&(k.body.isActive()&&k.enabled&&g.enabled)&&
(k.type===pc.fw.RIGIDBODY_TYPE_DYNAMIC?g.rigidbody.syncBodyToEntity():k.type===pc.fw.RIGIDBODY_TYPE_KINEMATIC&&g.rigidbody.updateKinematic(a))}var a=this.dynamicsWorld.getDispatcher(),b=a.getNumManifolds(),n;c={};for(d=0;d<b;d++){g=a.getManifoldByIndexInternal(d);n=g.getBody0();k=g.getBody1();n=Ammo.castObject(n,Ammo.btRigidBody);var m=Ammo.castObject(k,Ammo.btRigidBody),k=n.entity,m=m.entity;if(k&&m){var r=this._getCollisionFlags(k,m),z=this._getCollisionFlags(m,k);if(r||z){var C=g.getNumContacts();
if(0<C){var D,G=r&2||r&1,x=z&2||z&1;e.length=0;for(n=f.length=0;n<C;n++){var B=g.getContactPoint(n);r&32&&(D=this._createContactPointFromAmmo(B),this.fire("contact",new l(k,m,D)));G&&(D=D||this._createContactPointFromAmmo(B),e.push(D));x&&f.push(this._createReverseContactPointFromAmmo(B))}this._handleEntityCollision(k,m,e,r);this._handleEntityCollision(m,k,f,z)}}}}this._cleanOldCollisions()}});return{RIGIDBODY_TYPE_STATIC:"static",RIGIDBODY_TYPE_DYNAMIC:"dynamic",RIGIDBODY_TYPE_KINEMATIC:"kinematic",
